...
Default for **REPOSITORY** is **kratochj/mosquitto** (should change this) and for **TAG** is **mosquitto version (1.4.10 now)**.

Actually the command executed by make is

    docker build --no-cache -t kratochj/mosquitto:v1.4.10 .
...
(or whatever user u have configured...)
    ...
    $ docker run -ti --rm kratochj/mosquitto:v1.4.10 np -p secretpass
